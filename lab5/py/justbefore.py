...
class AVLTree:
    ...
    def printTree(self, node, level=0):
        if node is not None:
            ...
            print(" " * 4 * level + "->", node.stop_name, len(node.stops))
...
class PathFinder:
    ...
    def find_path(self, origin, destination):
        ...
        while not forward_exploration.is_empty() and not backward_exploration.is_empty():
            ...
            while forward_metrostop.prev_stop:
                forward_search_trip = Trip(forward_metrostop, forward_search_trip)
                                
                if forward_metrostop.stop_name == destination:
                    #make path
                    path = Path()
            ...
            while backward_metrostop.next_stop:
                backward_search_trip = Trip(backward_metrostop, backward_search_trip)
                                
                if backward_metrostop.stop_name == destination:
                    #make path
                    path = Path()
...
def main():
    ...
    # paths = path_finder.find_path("Janakpuri West", "Botanical Garden")
    # print(paths)
    # for path in paths:
    #     for stop in path.stops:
    #         print(stop.stop_name)
