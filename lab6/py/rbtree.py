...
class RedBlackTree():
    ...
    def print_formatted_tree(self, node, level=0):
        if node != self.TNULL:
            self.print_formatted_tree(node.left, level + 1)
            print(' ' * 8 * level + '-> ' + str(node.item) + str(node.color))
            self.print_formatted_tree(node.right, level + 1)
...
# Validate Red-Black Tree properties and other validations can be added as needed.
...
def check_black_height(node):
    ...
    assert not (node.color == True and node.parent and node.parent.color == True), f"Consecutive red nodes found at key {node.key}"
