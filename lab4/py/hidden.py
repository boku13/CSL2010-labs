...
from  last import BinaryTreeNode,TravelDesk,BinaryTree,BinarySearchTree
...
def test_searchkeyin_BST():
    ...
    assert node1 is None, "Node with key 1100 should not be found."
...
def test_successor_predecessor():
    ...
    # print(node.trip_node_ptr.departure_time)
    # print("bruh",node.departure_time)
    node = temp1.get_successor_node(node)
    print("successor :", node.departure_time)
    
    node1 = temp1.get_predecessor_node(node)
    print("predecessor :", node1.departure_time)
    
    assert node is not None and node.get_departure_time() == 1200, "Successor node with key 1000 has incorrect departure time."
